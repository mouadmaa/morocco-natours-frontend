...
const HomeLogin: FC<HomeLoginProps> = (props) => {
	...
	const HeroImage = [
		bigTabletView && !smallTabletView
			? `linear-gradient(105deg, rgba(255, 255, 255, 0.9) 0%, rgba(255, 255, 255, 0.9) 65%, transparent 65%)`
			: smallTabletView
				? `linear-gradient(105deg, rgba(255, 255, 255, 0.9) 0%, rgba(255, 255, 255, 0.9) 100%)`
	...
	return (
		<HomeBookingContainer>
			<BookingContentContainer>
				<BookingContainer fluid={HeroImage}>
					<BookingFormContainer>
						<form onSubmit={handleSubmit(onSubmit)}>
							...
							<Button type='buttonSmall'>{!loading ? 'Log In' : 'Loading...'}</Button>
						</form>
					</BookingFormContainer>
				</BookingContainer>
			</BookingContentContainer>
...
const query = graphql`
	{
		file(relativePath: { eq: "nature-5.jpg" }) {
			childImageSharp {
				fluid {
