...
const TourBooking: FC<TourBookingProps> = (props) => {
	...
	return (
		<SectionBookingContainer>
			<BookingContainer>
				{images.map((image, index) => <BookingImage key={index} index={index} src={image} alt={name} />)}
				<BookingContentContainer>
					<Heading type='Secondary'>What are you waiting for?</Heading>
					<BookingText>{duration} days, 1 adventure, Infinite memories, Make it yours today!</BookingText>
					{user ? (
						<Button onClick={handleClick}>{loading ? 'Processing...' : 'Book tour now!'}</Button>
					) : (
						<Button type='buttonSmall'>Log in to book tour</Button>
					)}
				</BookingContentContainer>
			</BookingContainer>
		</SectionBookingContainer>
